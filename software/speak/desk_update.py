...
def sync_mapped() -> dict[str, Any]:
    ...
    # ears: prefer flat ears.py used by ears.service --listen
    for up_rel in EARS_FLAT_CANDIDATES:
        src = UPSTREAM / up_rel
        if src.is_file():
            copied.append(_copy_file(src, LIVE / "ears.py"))
            # Also sync sibling helpers if present under software/ears/
            ears_dir = UPSTREAM / "software/ears"
            if ears_dir.is_dir():
                for helper in ("stt_xai.py", "webhook.py", "record.py", "__init__.py"):
                    h = ears_dir / helper
                    if h.is_file():
                        # Prefer flat helpers next to ears.py if live is flat;
                        # also keep under ears/ if that dir exists.
                        dest_flat = LIVE / helper
                        copied.append(_copy_file(h, dest_flat))
